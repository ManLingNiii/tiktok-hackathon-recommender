...
def run_fm(splits, k=16, lr=0.001, epochs=40, bs=8192, patience=4, seed=0, verbose=True):
    ...
    m = FM(dim, k=k, lr=lr, seed=seed) # 建立 FM 模型
    rng = np.random.default_rng(seed)
    ...
    for ep in range(1, epochs + 1):
        ...
        va = evaluate(uva, yva, m.predict(Xva))  # 計算GAUC, nDCG@5, primary
        if verbose:
    ...
    return {'valid': evaluate(uva, yva, m.predict(Xva)), 
            'test':  evaluate(ute, yte, m.predict(Xte))} #m.predict(Xva)對validation每筆資料產生一個分數
